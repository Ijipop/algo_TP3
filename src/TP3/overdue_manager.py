...
class OverdueManager:
        ...
        for loan in self.loan_manager.loans:
            if not loan.getReturned() and loan.getReturnDate() is None:
                ...
                if current_date > due_date:
                    ...
                    self.overdues.append(Overdue(loan, user, book, current_date))
    ...
    def send_report(self):
        ...
        for overdue in self.overdues:
            print(f"- {overdue.getBook().getTitle()} par {overdue.getUser().getName()} "
                f"({overdue.getDaysOverdue()} jours de retard)")
            
            self.notification_manager.notify(f"Retard: {overdue.getBook().getTitle()} - {overdue.getDaysOverdue()} jours")
